...
- `baseline_submit/`: 실제 제출 패키지
  ...
  - `requirements.txt`: scikit-learn 버전 불일치 이슈 있음 (아래 참고)
...
1. **`requirements.txt` scikit-learn 버전 수정 필요 (미적용)**. 로컬 Python이 3.9.6이라
   scikit-learn 1.6.1까지만 설치 가능한데, `requirements.txt`엔 `1.8.0`으로 적혀 있어
   실제 제출 시 `AttributeError: _RemainderColsList` 로 실패함(평가 서버는 Python 3.11).
   `1.6.1`로 고치는 게 맞다고 제안했고 사용자 확인 대기 중 — 다음 세션에서 바로 처리.
2. **CV 검증에서 val=2023 fold가 계속 0점** — overfitting 아니고 트렌드 추정 실패로 진단
   완료 (자세한 내용 `IMPROVEMENT_NOTES.md`). recency weighting 시도했지만 효과 미미.
   다음 시도 후보: 시즌별 선형회귀로 다음 시즌 예상 성공률을 구해 예측 평균을 보정하는
   calibration shift.
3. 노트북 재실행 후 `model/rf.pkl` → `baseline_submit/model/rf.pkl` 복사 필요 (자동 아님).
